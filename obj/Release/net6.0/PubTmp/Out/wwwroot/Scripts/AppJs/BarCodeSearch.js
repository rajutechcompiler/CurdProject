$(function () {
    ...
    $("#gridEdit").click(function () {
        ...
        if (selectedrows == null) {
            //showAjaxReturnMessage(vrApplicationRes['msgJsBarCodeSearchAtleastOneRow'], 'w');
            showAjaxReturnMessage(vrApplicationRes['msgJsBarCodelisttoEdit'], 'w');
            return;
        }
        else {
            $('#frmBarCodeSearchOrder').resetControls();
    ...
    $("#gridRemove").click(function () {
        ...
        if (selectedrows == null) {
            //showAjaxReturnMessage(vrApplicationRes['msgJsBarCodeSearchAtleastOneRow'], 'w');
            showAjaxReturnMessage(vrApplicationRes['msgJsBarCodelisttoRemove'], 'w');
            return;
        }
        else {
            $(this).confirmModal({
    ...
    $.post(urls.Common.GetColumnList, { pTableName: tableName, type : 0 })
               .done(function (data) {
                   if (data.isError === false) {
                       var pOutputObject = $.parseJSON(data.JsonString);
                       $('#FieldName').empty();
