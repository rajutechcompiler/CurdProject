...
function GetTABQUIKGridDataIntoJson(){
    ...
    for (var index = 0; index < rows.length; index++) {
        ...
        if (rows.length !== (index + 1) && row.TABQUIKField > "")
            sData += "{ \"TABQUIKField\": \"" + row.TABQUIKField.replace(/\\/g, "\\\\") + "\", \"TABFusionRMSField\": \"" + row.TABFusionRMSField + "\", \"Manual\": \"" + row.Manual.replace(/\\/g, "\\\\") + "\", \"Format\": \"" + row.Format.replace(/\\/g, "\\\\") + "\"},";
        else if (row.TABQUIKField > "")
            sData += "{ \"TABQUIKField\": \"" + row.TABQUIKField.replace(/\\/g, "\\\\") + "\", \"TABFusionRMSField\": \"" + row.TABFusionRMSField + "\", \"Manual\": \"" + row.Manual.replace(/\\/g, "\\\\") + "\", \"Format\": \"" + row.Format.replace(/\\/g, "\\\\") + "\"}";
    }
    sData += "]"
