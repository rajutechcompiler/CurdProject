...
function AddDBToList() {
    $.ajax({
        ...
        success: function (data) {
            var pOutputObject = $.parseJSON(data);
            var $select = $("#DBServer").selectize()
            var selectize = $select[0].selectize;
            selectize.clear();
            $(pOutputObject).each(function (i, v) {
                selectize.addOption({ value: pOutputObject[i], text: pOutputObject[i] });
            });
            //$('#DBServer').empty();
            //$(pOutputObject).each(function (i, v) {
            //    $('#DBServer').append($("<option>", { value: pOutputObject[i], html: pOutputObject[i] }));
            //});
            console.log("dblist:", pOutputObject)
        },
        error: function (xhr) {
            ShowErrorMessge();
        }
...
function SetSelectedData(dbName) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var $select = $("#DBServer").selectize()
            var selectize = $select[0].selectize;
            if ($("#DBServer option[value='" + abc +"']").length > 0){
                selectize.setValue(abc)
            } else {
                selectize.addOption({ value: abc, text: abc });
                selectize.setValue(abc)
            }
...
// Clear Db server dropdown values
function ClearDBServerDropdown() {
    var $select = $("#DBServer").selectize()
    var selectize = $select[0].selectize;
    selectize.clear();
}//Method complete.