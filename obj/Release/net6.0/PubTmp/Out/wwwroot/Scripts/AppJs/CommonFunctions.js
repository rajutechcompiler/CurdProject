...
function staticMenus(me) {
    ...
    switch (aId.toLowerCase()) {
        ...
        case 'tabquikfieldmapping':
            ...
            $('#title, #navigation').text("TABQUIK");
            break;
        //case 'azureadconfiguration':
        //    RedirectOnAccordian(urls.AzureAD.AzureADPartial);
        //    $('#title, #navigation').text(vrCommonRes['mnuAzureAD']);
        //    break;
...
function CheckTabLevelAccessPermission(vSecureObjectName, vSecureObjectType, vPassportPermissions) {
    ...
    $.ajax({
        ...
        if (result.isError === false) {
            bAccess = result.JsonString;
        } else {
            showAjaxReturnMessage(result.Msg, 'e');
        }
    });
    return bAccess;
}
