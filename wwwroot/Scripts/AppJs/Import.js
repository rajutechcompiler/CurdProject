...
$(function () {
    ...
    $('#btnNext').off().click(function (event) {
        $(".content-wrapper1").scrollTop(0, 0)  // Jira Number : FUS-10960
    ...
    $('#btnPrevious').off().click(function (event) {
        $(".content-wrapper1").scrollTop(0, 0)// jiraFUS-11278
        $('#btnNext').removeAttr('disabled', 'disabled');
        var currentStep = $('.nav-tabs > .active').find('a').attr('id');
...
function isFileHasBracket(filename) {
    var openBracket = filename.indexOf("[");
    var closeBracket = filename.indexOf("]")
    if (openBracket === -1 && closeBracket === -1) return false;

    if (openBracket > closeBracket) {
        return true;
    } else if (openBracket > -1 && closeBracket > -1) {
        return false;//we deal with this situation in the server side
    } else if (openBracket === -1 && closeBracket > -1) {
        return true;
    } else if (openBracket > -1 && closeBracket === -1) {
        return true;
    ...
    $.post(urls.Import.GetAvailableField, { currentLoad: currentLoad, tableName1: tableName1, changeFlag: changeFlag, IsSkipField: IsSkipField }, function (data) {
        if (data) {
            if (data.errortype == "s") {
                ...
                //check if list contain dueback dueback temporary fix by Moti Mashiah
                var selectFields = document.getElementById("bootstrap-duallistbox-selected-list_duallistbox_demo")
                for (var i = 0; i < selectFields.children.length; i++) {
                    if (selectFields.children.item(i).text == "<<TRACKING DUE DATE>>") {
                        selectFields.children[i].style.color = "red"
                        selectFields.children[i].innerHTML += `  ${vrClientsRes["msgBarCodeTrackingDueBackDateReq"]}`
                    }
                }
...
//`${item.Value.trim()} <span style="color:red">${vrClientsRes["msgBarCodeTrackingDueBackDateReq"]}</span>`;
