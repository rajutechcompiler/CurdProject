...
class TabLTApplication {
    ...
    checkLicense() {
        const url = `${window.location.origin}/Licenses/LEADTOOLS.lic.txt`;
        lt.RasterSupport.setLicenseUri(url, null, (r) => {
            if (r.result) {
                console.log("Has-license!");  
            } else {
                showAjaxReturnMessage(r.message, "e");
            }

            console.log(document.getElementById("DocumentServiceid").value)
            lt.Document.DocumentFactory.serviceHost = document.getElementById("DocumentServiceid").value;  //"http://localhost:40001"//window.location.origin;
    ...
    loadDocument(FilePath) {
        ...
        //v19: this.createOptions = new lt.Documents.UI.DocumentViewerCreateOptions();
        this.createOptions = new lt.Document.Viewer.DocumentViewerCreateOptions();
        ...
        //v19: this.documentViewer = lt.Documents.UI.DocumentViewerFactory.createDocumentViewer(this.createOptions);
        this.documentViewer = lt.Document.Viewer.DocumentViewerFactory.createDocumentViewer(this.createOptions);
        ...
        //v19: this.documentViewer.commands.run(lt.Documents.UI.DocumentViewerCommands.interactivePanZoom);
        this.documentViewer.commands.run(lt.Document.Viewer.DocumentViewerCommands.interactivePanZoom)

        //v19: this.documentViewer.view.preferredItemType = lt.Documents.UI.DocumentViewerItemType.svg;
        this.documentViewer.view.preferredItemType = lt.Document.Viewer.DocumentViewerItemType.svg;
        ...
        //v19: this.loadOption = new lt.Documents.LoadDocumentOptions();
        this.loadOption = new lt.Document.LoadDocumentOptions();
        ...
        //v19: var createOptions = new lt.Documents.CreateDocumentOptions();
        var createOptions = new lt.Document.CreateDocumentOptions();

        //v19: var c = lt.Documents.DocumentFactory.create(createOptions);
        var c = lt.Document.DocumentFactory.create(createOptions);

        c.name = "VirtualDocument";
        return new Promise((resolve, reject) => {
            lt.Document.DocumentFactory.loadFromUri(FilePath, this.loadOption)
                .done((document) => {
    ...
    viewPart(DocView, dialog) {
        ...
        this.headerToolbar_ViewMenu_click = {
            ...
            documentPropertiesVault: "#documentPropertiesVault",
            printDialog: "#print"
        };
        ...
        //Init dialogs
        //documents property for vault viewer
        $(this.headerToolbar_ViewMenu_click.documentPropertiesVault).on("click", function (e) {
            dialog.documentPropertiesVault_Dialog(DocView.document);
        });
        //pringing dialog
    ...
    LoadFromCache(funcall, formdata) {
        ...
        return new Promise((resolve, reject) => {
            ...
            $.ajax(ajaxOptions).done(function (data) {
                if (data.pageCount > 1) {
                    lt.Document.DocumentFactory.loadFromCache(data.documentid)
                        .done(function (doc) {
                            $("#imageViewerDiv").html("");
                            $("#thumbnailsTab").html("");
                            _this.createOptions = new lt.Document.Viewer.DocumentViewerCreateOptions();
                            ...
                            _this.documentViewer = lt.Document.Viewer.DocumentViewerFactory.createDocumentViewer(_this.createOptions);
                            ...
                            _this.loadOption = new lt.Document.LoadDocumentOptions();
...
class FunctionsCall {
    ...
    RunSpliter(ActiveFunc) {
        ...
        $("#spliter_id").on("mousedown", function (e) {
            ...
            leadtools.documentViewer.commands.run(lt.Document.Viewer.DocumentViewerCommands.interactivePanZoom);
        });
    }
    ...
    OnclickAttachmentSetupProperties(ActiveFunc = false, _thisElem) {
        ...
        //_this.recordId;
        //}
        var table = _thisElem.baseURI.split("DocumentViewer/")[1].split("?")[0];
        $("#showFilename_id").show();
        if (table == "Orphan") {
            $("#showFilename_id").html("<span></span><span></span><span style='color:#666666;font-size: 15px;text-transform: capitalize''>" + vrHTMLViewerRes["lblVaultAttachment"] + " - </span><span style='color:rgb(0, 161, 225)'>" + _this.CurrentFileFileName + "</span><input type='hidden' value='" + this.documentNumber + "' />");
        }
        else {
    ...
    RunImageFiles(CheckEditPermissions) {
        ...
        if (_this.ischeckOutToMe > 0 && _this.isCheckoutDesktop === 0) {
            _this.CancelcheckoutServerside(true).then(() => {
    ...
    UploadAttachment(ActiveFunc = false, files) {
        ...
        if (files.length == 1) {
            ...
            if (cFileFormat) {
                ...
                showAjaxReturnMessage(String.format(vrHTMLViewerRes["msgJsFileFormatNotSupported"], fileFormat), 'w');
            }
        } else {
    ...
    SaveAttachmentAfterAdd(ActiveFunc = false, formdata) {
        ...
        _this.ajaxCall = $.ajax(ajaxOptions).done(function (data) {
            ...
            if (func.isError == 'True') {
                showAjaxReturnMessage(func.ErrorMsg, "e")
                setTimeout(() => {
                    window.location.reload();
                }, 1000)
            }
    ...
    StartCreartingVirtualDocument() {
        ...
        var c = lt.Document.DocumentFactory.create(createOptions);
        c.name = "VirtualDocument";
        lt.Document.DocumentFactory.loadFromCache(leadtools.documentViewer.document.documentId)
            .done(function (doc) {
    ...
    AddPages(ActiveFunc = false, _thisElem) {
        ...
        var uploadPromise = lt.Document.DocumentFactory.uploadFile(fileUpload);
    ...
    AddpageToExistingDoc(cachFile) {
        ...
        lt.Document.DocumentFactory.loadFromUri(cachFile, loadDocumentOptions)
            .done(function (doc) {
                var docPage = doc.pages;
                for (var i = 0; i < docPage.count; i++) {
                    var addPages = docPage.item(i);
    ...
    CompleteVirtualDocAndSaveToCache() {
        ...
        return new Promise((resolve, reject) => {
            var _this = this;
            lt.Document.DocumentFactory.saveToCache(VirtualDocObject)
                .done(function () {
                    resolve();
    ...
    ConvertToTiff(convertFor) {
        var _this = this;
        var loadDocumentOptions = new lt.Document.LoadDocumentOptions();
        lt.Document.DocumentFactory.loadFromCache(leadtools.documentViewer.document.documentId, loadDocumentOptions)
            .done(function (doc) {
                ...
                jobData.rasterImageFormat = lt.Document.RasterImageFormat.tifJpeg422;
                doc.convert(jobData)
                    .done(function (result) {
    ...
    MagnifyGlassCall(ActiveFunc = false, _thisElem) {
        ...
        if (_this.MagnifyOn_Off == true) {
            ...
            leadtools.documentViewer.commands.run(lt.Document.Viewer.DocumentViewerCommands.interactiveMagnifyGlass);
    ...
    EnableOcr(ActiveFunc = false, _thisElem) {
        ...
        if (_this.OcrOn_Off == true) {
            ...
            leadtools.documentViewer.commands.run(lt.Document.Viewer.DocumentViewerCommands.interactiveRubberBand);
    ...
    GetTextOcr(searchArea, imageViewer, rubberBandMode) {
        var _this = this;
        var rest = `${lt.Document.DocumentFactory.serviceHost}/Ocr/GetText`;
    ...
    SearchForText(ActiveFunc = false, _thisElem) {
        ...
        if (Gettext.length >= 3) {
            ...
            var options = new lt.Document.Viewer.DocumentViewerFindText();
            ...
            lt.Document.Viewer.DocumentViewerText.foundTextFill = "rgba(252, 252, 3, .5)";
            var isFindingNext = true;
            var bottomOfLastPage = lt.Document.Viewer.DocumentViewerTextPosition.createEndOfPage(leadtools.documentViewer.pageCount);
            ...
            if (_thisElem == "pre") {
                ...
                if (leadtools.documentViewer.pageCount > 50 && _this.pagesArrayList.length !== 0) {
                    for (var i = 0; i < _this.pagesArrayList.length; i++) {
                        if (i === 0) {
                            ...
                            topOfFirstPage = lt.Document.Viewer.DocumentViewerTextPosition.createBeginOfPage(pageNumber);
                ...
                if (text.hasAnySelectedText) {
                    ...
                    if (isFindingNext)
                        options.manualStartPosition = lt.Document.Viewer.DocumentViewerTextPosition.createBeginOfPage(leadtools.documentViewer.currentPageNumber);
                    else
                        options.manualStartPosition = lt.Document.Viewer.DocumentViewerTextPosition.createEndOfPage(leadtools.documentViewer.currentPageNumber);
                }
            }
    ...
    PreviousTextFind(options, topOfFirstPage, bottomOfLastPage) {
        ...
        for (var i = 0; i < pre.length; i++) {
            var pageNum = pre[i];
            topOfFirstPage = lt.Document.Viewer.DocumentViewerTextPosition.createBeginOfPage(pageNum);
            options.beginPosition = topOfFirstPage;
            options.endPosition = bottomOfLastPage;
    ...
    ConvertDateAndTime(value) {
        var date = value.NoteDateTime.split("-");
        var year = date[0];
        var month = date[1]
    ...
    CheckDownloadDocPermission(viewId) {
        $.ajax({
            url: "/DocumentViewer/DownloadDocPermission",
            type: "POST",
            data: { viewid: viewId },
            async: false,
            success: function (data) {
                if (data.errortype == "s") {
                    ...
                    showAjaxReturnMessage(vrHTMLViewerRes["downloadpermissionmsg"], "w");
                    return false;
                } else {
                    console.log(data)
                    return false;
...
class Dialogs {
    ...
    documentPropertiesVault_Dialog(document) {
        func.GetIscheckOutNcheckOutToMeProps(true);
        document = leadtools.documentViewer.document;
        //get dialog id 
        if (leadtools.isDocumentPlaced != true) return;
    ...
    PrintDocument_Dialog(documentObject) {
        ...
        var orientation = lt.Document.Viewer.PrintOrientation.portrait;
        if (doc) {
            var size = doc.pages.item(0).size;
            if (size.width > size.height)
                orientation = lt.Document.Viewer.PrintOrientation.landscape;
        }
